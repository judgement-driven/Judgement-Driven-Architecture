...
## 14. Business Journey（BJ）の分割基準

### 背景

文化アディックのBJ04（制作進行）のDiscoveryを検討する中で、
素材管理と制作進行を別BJに分割すべきかという議論が生じた。

その結果、BJは工程や担当部署ではなく、
**業務目的（最終的に生み出す成果物・価値ある状態）**を基準として定義すべきという整理に至った。

### 定義

Business Journey（BJ）は、

> **業務として意味のある成果物（価値ある状態）を生み出すための判断群**

として定義する。

BJは工程単位ではない。

素材受領や素材確認などの中間工程ではなく、
業務として意味のある成果物（価値ある状態）までを一つのBJとして扱う。

そのため、業務工程が複数存在していても、
**同一の成果物（価値ある状態）を目指すものであれば、原則として一つのBJとして扱う。**

### Discovery時の考え方

BJは最初から細かく分割することを目的としない。

まず業務目的を基準にBJを定義し、DiscoveryによってJPを抽出する。

Discoveryの結果、複数の独立した業務目的を持つことが明らかになった場合は、
後からBJを再構成・分割してよい。
...
- BJは工程単位ではない
- BJは担当部署単位でもない
- **BJの分割基準は「業務目的（成果物・価値ある状態）の変化」である**
