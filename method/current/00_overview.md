...
-   企業活動はプロセスではなく、Case / Proposal
    の状態遷移として構造化される
-   業務改善ではなく、判断能力の強化を目的とする
-   AIは行動ではなく、判断材料学習・判断再現・判断委譲に使う
...
JDAでは、 BJをJP発見のスコープとして使用する。

------------------------------------------------------------------------

## Judgement Journey（JJ）

JJとは、 JPの実行連鎖によって形成される 判断実行構造である。

JPはDiscovery時にBJをスコープとして発見されるが、
発見後はBJから独立した共通判断資産として扱われる。

JJは、 ハーネス上でCase / Proposalの 状態・属性・判断材料が
どのように更新されていくかを表す 判断実行視点のJourneyである。
...
JPは特定BJに従属せず、 複数BJから共有されうる共通判断資産として扱う。
...
JDAでは、 以下のLearning Cycleを前提とする。

------------------------------------------------------------------------

## 8.1 Learning Foundation

初期段階では、判断材料を継続的に収集・整備し、学習可能な基盤を構築する。

------------------------------------------------------------------------

## 8.2 Judgement Material Learning
...
十分な学習後、 判断は段階的にAIへ委譲される。

------------------------------------------------------------------------

# 9. 実装における補足（Judgement Harness）

JDAは特定技術を前提としない。

しかし、 判断を実行可能かつ学習可能にするためには、
共通実行基盤が必要となる。
...
> JPをハーネス内部で取得するのではなく、 外部定義として注入する実装方式
...
# 12. 変更履歴
  ...
  v1.4                                Learning Loop / Delegation /
                                      Venture Judgement 強化

  v1.5                                Judgement Injection /
                                      JP共有構造の検討

  v1.6                                判断実行 + 学習アーキテクチャへ拡張
                                      / JJ再定義 / execute_jp統合

  v1.7                                Judgement Scorecard・JSC（State
                                      Chart）・Judgement
                                      Snapshot・Learning Foundation
                                      を反映 / Judgement
                                      Harnessへ名称統一
  -----------------------------------------------------------------------
